...
class ChatViewProvider {
    ...
    openPanel() {
        ...
        this.panel = vscode.window.createWebviewPanel('devpilot.chatPanel', 'DevPilot', vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
        this.webview = this.panel.webview;
    ...
    async onSend(text) {
        ...
        if (!this.sessionId) {
            ...
            try {
                const s = await api_1.api.newSession();
        ...
        try {
            ...
            const res = await api_1.api.chat([{ role: 'user', content: text }], this.sessionId);
    ...
    async onDiagnostics() {
        ...
        await this.onSend(`Read this project context, find the errors across the whole project, and propose fixes. Use VS Code diagnostics as the priority list. If files should change, return a short explanation followed by complete replacement code blocks labeled with each file path.\n\n${context}`);
    }
    async onClear() {
        if (this.sessionId) {
            await api_1.api.deleteSession(this.sessionId).catch(() => { });
    ...
    extractLabeledCodeBlocks(markdown) {
        const blocks = [];
        const pattern = /(?:^|\n)(?:File|Path):\s*([^\r\n`]+)\r?\n```[\w.+-]*\r?\n([\s\S]*?)```/g;
    ...
    getWorkspaceDiagnostics(rootUri) {
    ...
    async findProjectSourceFiles(rootUri) {
        const files = await vscode.workspace.findFiles(new vscode.RelativePattern(rootUri, '**/*'), new vscode.RelativePattern(rootUri, '**/{node_modules,bin,obj,out,dist,build,.git}/**'), 80);
        return files
            .filter(uri => this.isUsefulSourceFile(uri.fsPath))
            .sort((a, b) => this.sourcePriority(a.fsPath) - this.sourcePriority(b.fsPath));
    }
    async readDocumentText(uri) {
    ...
    formatDiagnosticsForUri(uri, limit) {
